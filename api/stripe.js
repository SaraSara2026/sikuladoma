...
const PLAN_NAMES = { plus: 'Plus', profi: 'Profi', top: 'Top Šikula' };
...
async function handleCheckout(req, res, me) {
  const { plan } = req.body ?? {};
  if (!['plus', 'profi', 'top'].includes(plan)) {
    return res.status(400).json({ error: 'Neplatný plán. Povoleno: plus, profi, top.' });
  }
  ...
  const origin = req.headers.origin || req.headers.referer?.replace(/\/$/, '') || 'https://sikuladoma.cz';

  // Pokud šikula již má Stripe zákazníka, přiřadíme session k němu
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${origin}/dashboard?stripe=success&plan=${plan}&session_id={CHECKOUT_SESSION_ID}`,
...
async function processEvent(event) {
  switch (event.type) {
    ...
    case 'checkout.session.completed': {
      ...
      if (!userId || !PLAN_NAMES[plan]) break;

      const customerId     = session.customer;
      const subscriptionId = session.subscription;

      // Načteme subscription pro expires_at
      let expiresAt = null;
      if (subscriptionId) {
        try {
          const stripe = getStripe();
      ...
      await sql`
        ...
        WHERE id = ${userId}
      `;
      console.log(`[stripe] ✅ User ${userId} upgradován na ${plan}`);
      break;
    }
    ...
    case 'customer.subscription.updated': {
      ...
      // Zjistíme plán z price ID
      const priceId = sub.items?.data?.[0]?.price?.id;
      const plan = Object.entries(PRICE_IDS).find(([, fn]) => fn() === priceId)?.[0];
      if (!plan) break;
      ...
      await sql`
        ...
        WHERE id = ${userId}
      `;
      console.log(`[stripe] ✅ User ${userId} plán aktualizován na ${plan}`);
      break;
    }
    ...
    case 'customer.subscription.deleted': {
      ...
      await sql`
        ...
        WHERE id = ${userId}
      `;
      console.log(`[stripe] ℹ️  User ${userId} zrušil předplatné → start`);
      break;
    }

    case 'invoice.payment_failed': {
      const invoice = event.data.object;
      // Stripe sám zašle email zákazníkovi o selhání platby.
      // Pro nás zatím jen logujeme — plán ponecháme do konce okresu.
