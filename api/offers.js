...
export default async function handler(req, res) {
  try {
    if (req.method === 'POST') return createOffer(req, res);
    if (req.method === 'GET')  return listOffers(req, res);
...
// ─── POST /api/offers ───────────────────────────────────────────────────────
// Šikula posílá nabídku na poptávku. Vyžaduje role 'sikula'.
async function createOffer(req, res) {
  ...
  // Existuje poptávka a je otevřená?
  ...
  try {
    const [row] = await sql`
      ...
      RETURNING *
    `;
    // Posun poptávku do 'in_progress' (= má aspoň jednu nabídku)
...
// ─── GET /api/offers?order_id= ──────────────────────────────────────────────
// - Bez order_id: šikula vidí své nabídky, customer/admin chybu
// - S order_id: vidí customer dané poptávky, šikula který nabídl, admin
async function listOffers(req, res) {
  ...
  // Šikula vidí jen vlastní nabídku na tuto poptávku; customer/admin vidí všechny.
