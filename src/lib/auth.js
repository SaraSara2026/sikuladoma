...
export async function apiRegister({ email, password, name, role, phone, city, services, worker_type, street, zip, city_area, ico, has_liability_insurance }) {
  const res = await fetch('/api/auth/register', opts({
    method: 'POST',
    headers: j,
    body: JSON.stringify({ email, password, name, role, phone, city, services, worker_type, street, zip, city_area, ico, has_liability_insurance }),
  }));
  if (!res.ok) throw new Error(await parseError(res));
  return res.json();
}
