...
export const ordersApi = {
  ...
  get:    (id) => fetch(`/api/orders/${id}`, opts()).then(unwrap),
...
export const reviewsApi = {
  ...
  myReviews: ()         => fetch('/api/reviews?my_reviews=1',          opts()).then(unwrap),
};

// ─── Users (veřejné) ────────────────────────────────────────────────────────
export const usersApi = {
  publicProfile: (id) => fetch(`/api/users/${id}`, opts()).then(unwrap),
  listSikulove: ({ category, city, search, verified, minRating } = {}) => {
